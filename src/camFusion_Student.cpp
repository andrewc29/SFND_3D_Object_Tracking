...
{
    std::vector<double> distanceRatios; //Compute distance ratio for TTC calculation later on
    double minDistanceThreshold = 100.0; //Minimum distance between keypoints
    ...
    { 
        ...
        { 
            ...
            //Prevent Division by a near zero number
    ...
    // compute camera-based TTC from minumum distance
    double dT = 1 / frameRate;
    TTC = -dT / (1 - medianDistRatio);

}

void HelperSortLidarPoints(std::vector<LidarPoint> &lidarPoints)
{
    std::sort(lidarPoints.begin(), lidarPoints.end(), [](LidarPoint a, LidarPoint b) {
        return a.x < b.x;  // sort ascending
    });
}
...
{
    HelperSortLidarPoints(lidarPointsPrev);
    HelperSortLidarPoints(lidarPointsCurr);
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame) {
    ...
    for (const auto &match : matches) {
        ...
        // Find which bounding box in the previous frame contains the prevKpt
        int prevBoxID = -1;
        ...
        // Find which bounding box in the current frame contains the currKpt
        int currBoxID = -1;
    ...
    for (const auto &prevBox : prevFrame.boundingBoxes) {
        int bestMatchCurrBoxID = -1;
        int maxCount = 0;

        for (const auto &count : bbMatchCounts) {
            if (count.first.first == prevBox.boxID && count.second > maxCount) {
                bestMatchCurrBoxID = count.first.second;
                maxCount = count.second;
            }
        }

        if (bestMatchCurrBoxID != -1) {
            bbBestMatches[prevBox.boxID] = bestMatchCurrBoxID;
        }
    }
}